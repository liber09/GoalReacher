...
class ToDoViewModel: ObservableObject {
    ...
    func save(title: String, wantsRemainder: Bool, lastDate: Date, streakDays: Int, selectedDate: Date, done: Bool){
        guard let user = auth.currentUser else {return}
        let todoRef = db.collection("users").document(user.uid).collection("todos")
        let todo = ToDoItem(title: title, wantsRemainder: wantsRemainder, lastDate: Date(),streakDays: 0, selectedDate: Date(), done: false, daysToDo: daysToDo)
                do {
